...
typedef struct {
	int prevq;
	strings prompt[MaxPromptLines + 1];
    strings qtext[MaxLinesPerQuestion + 1];
...
static boolean ReadOneQuestion(FILE *infile, courseDB course);
static void ReadPromptText(FILE *infile, questionT q);
static void ReadQuestionText(FILE *infile, questionT q);
static void ReadAnswers(FILE *infile, questionT q);
static FILE *OpenUserFile(strings prompt, strings mode);
static void ProcessCourse(courseDB course);
static void ShowPrompt(questionT q);
static void AskQuestion(questionT q);
static int FindAnswer(strings ans, questionT q);
...
static courseDB ReadDataBase(void)
{
    ...
    infile = OpenUserFile("Enter name of course: ", "r");//please input cs.dat or advent.dat
    course = New(courseDB);
 ...
 * Usage: ReadPromptText(infile, question);
 * ------------------------------------------
 * This function reads the text of the prompt into the
 * question data structure, which must have been allocated
 * by the caller.  The end of the prompt text is signaled
 * by a blank line.
 */
...
static void ReadPromptText(FILE *infile, questionT q)
{
	...
	while ((line = ReadLine(infile)) != NULL
		&& (len = StringsLength(line)) != 0) {
		if (nlines == MaxPromptLines) {
			Error("Too many lines");
		}
		q->prompt[nlines] = line;
		nlines++;
	}
	q->prompt[nlines] = NULL;
}
...
static void ReadQuestionText(FILE *infile, questionT q)
{
    ...
    while (TRUE) {
        line = ReadLine(infile);
        if (StringsEqual(line, EndMarker)) break;
...
static void ReadAnswers(FILE *infile, questionT q)
{
    ...
    while ((line = ReadLine(infile)) != NULL
        ...
        ans = SubStrings(line, 0, cpos - 1);
        nextq = StringsToInteger(SubStrings(line, cpos+1, len-1));
...
static void ProcessCourse(courseDB course)
{
    ...
    while (qnum != 0) {
        ...
        if (index == -1) {
			...
			course->questions[qnum]->prevq = tmp;
        }
    }
}
 ...
 * Usage: ShowPrompt(q);
 * ----------------------
 * This function show the prompt message.
 */
...
static int FindAnswer(strings ans, questionT q)
{
    ...
    for (i = 0; i < q->nAnswers; i++) {
        if (StringsEqual(ans, q->answers[i].ans)) return (i);
    }
    return (-1);
}
