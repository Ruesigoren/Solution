...
static int FindTheTopString(strings array[], int low, int high)
{
	...
	spos = low;
	for (i = low; i <= high; i++) {
		if (StringsCompare(ConvertToUpperCase(array[spos]), ConvertToUpperCase(array[i])) > 0) {  //非零为真
	  //if (StringsCompare(ConvertToUpperCase(array[spos]), ConvertToUpperCase(array[i]))) {  //错误写法
			spos = i;
		}
	}
	return (spos);
