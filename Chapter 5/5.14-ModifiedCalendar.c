/*
 * File: 5.14-ModifiedCalendar.c
 * -----------------------------
 * Changed the implementation of the FirstDayOfMonth function
 * so that the calendar program can be used for years prior to 1900.
 */
...
int FirstDayOfMonth(int month, int year)
{
	...
	if (year >= 1900) {
		for (i = 1900; i < year; i++) {
			weekday = (weekday + 365) % 7;  //(weekday + 365 % 7) % 7;
			if (IsLeapYear(i)) weekday = (weekday + 1) % 7;  //(weekday + 1 % 7) % 7;
		}
	}
	else {
		for (i = year; i < 1900; i++) {
			weekday = (weekday + 6) % 7;  //(weekday + (7 - 365 % 7)) % 7;
										  //find the complement;
			                              //avoid using % with negative operands.
			if (IsLeapYear(i)) weekday = (weekday + 6) % 7;  //(weekday + (7 - 1 % 7)) % 7;
		}
	}
	for (i = 1; i < month; i++) {
