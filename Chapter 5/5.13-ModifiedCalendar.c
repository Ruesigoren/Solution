/*
 * File: 5.13-ModifiedCalendar.c
 * -----------------------------
 * Modified the program so that the year can be entered using only two digits,
 * in which case the year is assumed to be in the twentieth century.
 * Thus, if the user entered 94,
 * the program would generate a calendar for 1994.
 */
...
void GiveInstructions(void)
{
	...
	printf("please use two numbers for the year.\n");
	printf("eg: 94 represents 1994.\n");
}
...
int GetYearFromUser(void)
{
	...
	while (TRUE) {
		printf("Which year? ");
		year = GetInteger();
		if ((year >= 0) && (year < 100)) return (1900 + year);
		printf("The year must be at 0 - 99.\n");
	}
}
